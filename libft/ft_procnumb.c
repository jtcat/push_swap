...
/*   Updated: 2022/12/28 20:32:17 by joaoteix         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "utils.h"

void	ft_putpadding(char c, size_t len)
{
	while (len-- > 0)
		ft_putchar_fd(c, 1);
}
...
void	ft_procnumb(t_formspec *specs, unsigned long int num, int sign)
{
	...
	padding = specs->field_w - specs->numlen - ft_strlen(specs->prefix) \
		- specs->zeros;
	if (!specs->flags['-'])
		ft_putpadding(' ', padding);
	ft_putstr_fd(specs->prefix, 1);
	ft_putpadding('0', specs->zeros);
